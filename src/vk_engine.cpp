...
void VulkanEngine::draw()
{
    ...
    VK_CHECK(vkAcquireNextImageKHR(_device, _swapchain, 1000000000, get_current_frame()._swapchainSemaphore, nullptr,
                                   &swapchainImageIndex));
    // choose the render-finished semaphore tied to this image
    VkSemaphore imageRenderFinishedSemaphore = _imageRenderSemaphores[swapchainImageIndex];
    //< draw_2
    ...
    VkSemaphoreSubmitInfo signalInfo =
        vkinit::semaphore_submit_info(VK_PIPELINE_STAGE_2_ALL_GRAPHICS_BIT, imageRenderFinishedSemaphore);

    VkSubmitInfo2 submit = vkinit::submit_info(&cmdinfo, &signalInfo, &waitInfo);

    // submit command buffer to the queue and execute it.
    // _renderFence will now block until the graphic commands finish execution
    // it will be signaled when the command buffers finish execution. This allows us to know when it is safe for the
    // command buffer to be reused. Now on the next frame, the CPU will wait for this command buffer to finish executing
    ...
    // we want to wait on the imageRenderFinishedSemaphore for that,
    ...
    presentInfo.pWaitSemaphores = &imageRenderFinishedSemaphore;
...
void VulkanEngine::create_swapchain(uint32_t width, uint32_t height)
{
    ...
    for (size_t i = 0; i < _imageRenderSemaphores.size(); i++)
    {
        VK_CHECK(vkCreateSemaphore(_device, &semaphoreCreateInfo, nullptr, &_imageRenderSemaphores[i]));
    }
}
...
void VulkanEngine::destroy_swapchain()
{
    ...
    for (VkSemaphore sem : _imageRenderSemaphores)
    {
