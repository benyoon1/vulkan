...
void VulkanEngine::init()
{
    ...
    mainCamera.position = glm::vec3(5.f, 5.f, 10.f);
...
void VulkanEngine::init_background_pipelines()
{
    ...
    if (!vkutil::load_shader_module(shader_path("gradient_color.comp.spv").c_str(), _device, &gradientShader))
    {
        fmt::print("Error when building the compute shader \n");
    }

    VkShaderModule skyShader;
    if (!vkutil::load_shader_module(shader_path("sky.comp.spv").c_str(), _device, &skyShader))
    {
        fmt::print("Error when building the compute shader\n");
    }
...
void VulkanEngine::init_descriptors()
{
    ...
    }
    {
        ...
        std::array<VkDescriptorBindingFlags, 2> flagArray{0, VK_DESCRIPTOR_BINDING_VARIABLE_DESCRIPTOR_COUNT_BIT |
...
void GLTFMetallic_Roughness::build_pipelines(VulkanEngine* engine)
{
    ...
    if (!vkutil::load_shader_module(shader_path("mesh_pbr.frag.spv").c_str(), engine->_device, &meshFragShader))
    {
