...
// works when launched from either the repo root or the build/bin directory.
static std::string shader_path(const char *filename) {
  namespace fs = std::filesystem;
  fs::path from_bin = fs::path("../shaders") / filename; // running from bin/
...
void VulkanEngine::init_background_pipelines() {
  ...
  if (!vkutil::load_shader_module(
          shader_path("gradient_color.comp.spv").c_str(), _device,
          &gradientShader)) {
    fmt::print("Error when building the compute shader \n");
  }

  VkShaderModule skyShader;
  if (!vkutil::load_shader_module(shader_path("sky.comp.spv").c_str(), _device,
                                  &skyShader)) {
    fmt::print("Error when building the compute shader\n");
  }
  ...
  VkPipelineLayout pipelineLayout = _gradientPipelineLayout;
  _mainDeletionQueue.push_function([=, this]() {
    vkDestroyPipeline(this->_device, skyPipeline, nullptr);
    vkDestroyPipeline(this->_device, gradientPipeline, nullptr);
...
void VulkanEngine::draw() {
  ...
  _drawExtent.height =
      std::min(_swapchainExtent.height, _drawImage.imageExtent.height) * 1.f;
  _drawExtent.width =
      std::min(_swapchainExtent.width, _drawImage.imageExtent.width) * 1.f;
  ...
  VkSemaphoreSubmitInfo signalInfo =
      vkinit::semaphore_submit_info(VK_PIPELINE_STAGE_2_ALL_GRAPHICS_BIT,
                                    _presentSemaphores[swapchainImageIndex]);
...
void GLTFMetallic_Roughness::build_pipelines(VulkanEngine *engine) {
  ...
  if (!vkutil::load_shader_module(shader_path("mesh_pbr.frag.spv").c_str(),
                                  engine->_device, &meshFragShader)) {
    fmt::println("Error when building the triangle fragment shader module");
  }

  VkShaderModule meshVertexShader;
  if (!vkutil::load_shader_module(shader_path("mesh.vert.spv").c_str(),
...
TextureID TextureCache::AddTexture(const VkImageView &image,
                                   VkSampler sampler) {
